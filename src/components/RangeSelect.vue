<template>
  <div>
    ...
    <label for="range">
      <input @input="$emit('onPriceSelect', $event.target.value), updateMaxPrice($event)" type="range" name="range" id="range" min="0" max="300" step="5" value="175"/>
    </label>
    <output for="range" class="output">Maximum Price: ${{ maxPrice }}</output>
  </div>
</template>
